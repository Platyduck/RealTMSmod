...
import net.mcreator.tmsmod.item.ItemRedPlasticBerries;
...
public class BlockPlasticBerryBush extends ElementsTmsModMod.ModElement {
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0,
				new ModelResourceLocation("tms_mod:plastic_berry_bush", "inventory"));
	}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("tms_mod:plastic_taiga")))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
		for (int i = 0; i < 4; i++) {
	...
	public static class BlockCustomFlower extends BlockFlower {
		public BlockCustomFlower() {
			...
			setResistance(0F);
			setLightLevel(0F);
			setUnlocalizedName("plastic_berry_bush");
			setRegistryName("plastic_berry_bush");
		}
		...
		public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
			drops.add(new ItemStack(ItemRedPlasticBerries.block, (int) (3)));
		}
