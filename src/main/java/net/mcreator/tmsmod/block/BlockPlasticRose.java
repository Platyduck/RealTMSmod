...
public class BlockPlasticRose extends ElementsTmsModMod.ModElement {
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(Item.getItemFromBlock(block), 0, new ModelResourceLocation("tms_mod:plastic_rose", "inventory"));
	}
	...
	public void generateWorld(Random random, int chunkX, int chunkZ, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("tms_mod:joshian_tunnel_park")))
			biomeCriteria = true;
		if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("tms_mod:plastic_birch_forest")))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
		for (int i = 0; i < 5; i++) {
	...
	public static class BlockCustomFlower extends BlockFlower {
		public BlockCustomFlower() {
			...
			setResistance(0F);
			setLightLevel(0F);
			setUnlocalizedName("plastic_rose");
			setRegistryName("plastic_rose");
		}
