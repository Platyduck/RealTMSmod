...
package net.mcreator.tmsmod.world.structure;
...
import net.mcreator.tmsmod.world.WorldKillBoarsDimention;
import net.mcreator.tmsmod.ElementsTmsModMod;

import java.util.Random;

@ElementsTmsModMod.ModElement.Tag
public class StructureKillBoarsRuins1 extends ElementsTmsModMod.ModElement {
	public StructureKillBoarsRuins1(ElementsTmsModMod instance) {
		super(instance, 1044);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (dimID == WorldSnurbum4.DIMID) {
			...
			isNetherType = WorldSnurbum4.NETHER_TYPE;
		}
		if (dimID == WorldKillBoarsDimention.DIMID) {
			dimensionCriteria = true;
			isNetherType = WorldKillBoarsDimention.NETHER_TYPE;
		}
		...
		if ((random.nextInt(1000000) + 1) <= 10000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("tms_mod:snurbum_highlands")))
					biomeCriteria = true;
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("tms_mod:crag_peaks")))
					biomeCriteria = true;
				if (!biomeCriteria)
					continue;
				if (world.isRemote)
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("tms_mod", "kbruins"));
				if (template == null)
					return;
