...
public class StructureKobeCastle extends ElementsTmsModMod.ModElement {
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (dimID == WorldKobePlanet.DIMID) {
			...
			isNetherType = WorldKobePlanet.NETHER_TYPE;
		}
		...
		if ((random.nextInt(1000000) + 1) <= 500) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (Biome.REGISTRY.getNameForObject(biome).equals(new ResourceLocation("tms_mod:kobe_open_plains")))
					biomeCriteria = true;
				if (!biomeCriteria)
					continue;
				if (world.isRemote)
				...
				Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
						new ResourceLocation("tms_mod", "kobecastle"));
				if (template == null)
					return;
