...
import net.mcreator.tmsmod.block.BlockBurnwood;
import net.mcreator.tmsmod.block.BlockBurnstone;
import net.mcreator.tmsmod.block.BlockBurnleaf;
import net.mcreator.tmsmod.ElementsTmsModMod;

import java.util.Random;

@ElementsTmsModMod.ModElement.Tag
public class BiomeBurnlands extends ElementsTmsModMod.ModElement {
	@GameRegistry.ObjectHolder("tms_mod:burnlands")
	public static final BiomeGenCustom biome = null;
	public BiomeBurnlands(ElementsTmsModMod instance) {
		super(instance, 1048);
	}
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Burnlands").setRainfall(0.5F).setBaseHeight(0.1F).setWaterColor(-26368).setHeightVariation(0.2F)
					.setTemperature(0.5F));
			setRegistryName("burnlands");
			topBlock = BlockBurnstone.block.getDefaultState();
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockBurnstone.block.getDefaultState().getBlock() || ground == BlockBurnstone.block.getDefaultState().getBlock())
							&& (ground2 == BlockBurnstone.block.getDefaultState().getBlock()
									|| ground2 == BlockBurnstone.block.getDefaultState().getBlock())))
						return false;
					IBlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						world.setBlockState(position.down(), BlockBurnstone.block.getDefaultState(), 2);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == BlockBurnleaf.block.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, BlockBurnleaf.block.getDefaultState());
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.AIR.getDefaultState().getBlock()
									|| state.getBlock() == BlockBurnleaf.block.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), BlockBurnwood.block.getDefaultState());
								if (genh > 0) {
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockBurnwood.block.getDefaultState().getBlock()
					|| blockType == BlockBurnleaf.block.getDefaultState().getBlock() || blockType == BlockBurnstone.block.getDefaultState().getBlock()
					|| blockType == BlockBurnstone.block.getDefaultState().getBlock();
		}

		@Override
		protected void setDirtAt(World world, BlockPos pos) {
			if (world.getBlockState(pos).getBlock() != BlockBurnstone.block.getDefaultState().getBlock())
				this.setBlockAndNotifyAdequately(world, pos, BlockBurnstone.block.getDefaultState());
		}
