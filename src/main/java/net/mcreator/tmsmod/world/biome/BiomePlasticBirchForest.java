...
import net.mcreator.tmsmod.block.BlockPlasticLogBirch;
...
public class BiomePlasticBirchForest extends ElementsTmsModMod.ModElement {
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Artificial Birch Park").setRainfall(0.6F).setBaseHeight(0.1F).setWaterColor(-16763393)
					.setHeightVariation(0.2F).setTemperature(1F));
			setRegistryName("plastic_birch_forest");
	...
	static class CustomTree extends WorldGenAbstractTree {
					...
					if (position.getY() < world.getHeight() - height - 1) {
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.AIR.getDefaultState().getBlock()
									|| state.getBlock() == BlockPlasticLeaves.block.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), BlockPlasticLogBirch.block.getDefaultState());
								if (genh > 0) {
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockPlasticLogBirch.block.getDefaultState().getBlock()
