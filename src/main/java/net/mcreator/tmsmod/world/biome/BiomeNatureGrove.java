...
public class BiomeNatureGrove extends ElementsTmsModMod.ModElement {
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Nature Grove").setRainfall(0.5F).setBaseHeight(0.1F).setWaterColor(-16737793).setHeightVariation(0.2F)
					.setTemperature(0.5F));
			setRegistryName("nature_grove");
	...
	static class CustomTree extends WorldGenAbstractTree {
					...
					if (position.getY() < world.getHeight() - height - 1) {
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.VINE.getDefaultState().getBlock()
									|| state.getBlock() == BlockNatureLeaves.block.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), BlockOvergrownLog.block.getDefaultState());
								if (genh > 0) {
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockOvergrownLog.block.getDefaultState().getBlock()
					|| blockType == BlockNatureLeaves.block.getDefaultState().getBlock() || blockType == Blocks.DIRT.getStateFromMeta(2).getBlock()
