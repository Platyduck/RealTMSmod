...
import net.mcreator.tmsmod.block.BlockGrassyLeaves;
import net.mcreator.tmsmod.ElementsTmsModMod;

import java.util.Random;

@ElementsTmsModMod.ModElement.Tag
public class BiomeMossyHollows extends ElementsTmsModMod.ModElement {
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Mossy Hollows").setRainfall(0.5F).setBaseHeight(0.1F).setWaterColor(-15829248).setHeightVariation(0.2F)
					.setTemperature(0.5F));
			setRegistryName("mossy_hollows");
			topBlock = BlockMoss.block.getDefaultState();
			fillerBlock = BlockMoss.block.getDefaultState();
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockMoss.block.getDefaultState().getBlock() || ground == BlockMoss.block.getDefaultState().getBlock())
							&& (ground2 == BlockMoss.block.getDefaultState().getBlock() || ground2 == BlockMoss.block.getDefaultState().getBlock())))
						return false;
					IBlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						world.setBlockState(position.down(), BlockMoss.block.getDefaultState(), 2);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == BlockGrassyLeaves.block.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, BlockGrassyLeaves.block.getDefaultState());
										}
									}
								}
							}
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockMangroveLog.block.getDefaultState().getBlock()
					|| blockType == BlockGrassyLeaves.block.getDefaultState().getBlock() || blockType == BlockMoss.block.getDefaultState().getBlock()
					|| blockType == BlockMoss.block.getDefaultState().getBlock();
		}

		@Override
		protected void setDirtAt(World world, BlockPos pos) {
			if (world.getBlockState(pos).getBlock() != BlockMoss.block.getDefaultState().getBlock())
				this.setBlockAndNotifyAdequately(world, pos, BlockMoss.block.getDefaultState());
		}
