...
public class BiomeKobeBluelands extends ElementsTmsModMod.ModElement {
	@GameRegistry.ObjectHolder("tms_mod:kobe_bluelands")
	public static final BiomeGenCustom biome = null;
	public BiomeKobeBluelands(ElementsTmsModMod instance) {
		super(instance, 1372);
	}
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Kobe Plains").setRainfall(0.5F).setBaseHeight(0.1F).setWaterColor(-16763905).setHeightVariation(0.2F)
					.setTemperature(0.5F));
			setRegistryName("kobe_bluelands");
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == Blocks.GRASS.getDefaultState().getBlock() || ground == BlockBlueslate.block.getDefaultState().getBlock())
							&& (ground2 == Blocks.GRASS.getDefaultState().getBlock()
									|| ground2 == BlockBlueslate.block.getDefaultState().getBlock())))
						return false;
					IBlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						world.setBlockState(position.down(), BlockBlueslate.block.getDefaultState(), 2);
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.AIR.getDefaultState().getBlock()
									|| state.getBlock() == Blocks.AIR.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), BlockBlueslate.block.getDefaultState());
								if (genh > 0) {
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockBlueslate.block.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == Blocks.GRASS.getDefaultState().getBlock()
					|| blockType == BlockBlueslate.block.getDefaultState().getBlock();
		}

		@Override
		protected void setDirtAt(World world, BlockPos pos) {
			if (world.getBlockState(pos).getBlock() != BlockBlueslate.block.getDefaultState().getBlock())
				this.setBlockAndNotifyAdequately(world, pos, BlockBlueslate.block.getDefaultState());
		}
