...
import net.mcreator.tmsmod.block.BlockCrubstone;
import net.mcreator.tmsmod.ElementsTmsModMod;

import java.util.Random;

@ElementsTmsModMod.ModElement.Tag
public class BiomeAbbyFields extends ElementsTmsModMod.ModElement {
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Abby Fields").setRainfall(0.5F).setBaseHeight(0.1F).setWaterColor(-10092391).setHeightVariation(0.2F)
					.setTemperature(0.5F));
			setRegistryName("abby_fields");
			topBlock = BlockCrubstone.block.getDefaultState();
			fillerBlock = BlockCrubstone.block.getDefaultState();
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockCrubstone.block.getDefaultState().getBlock() || ground == BlockCrubstone.block.getDefaultState().getBlock())
							&& (ground2 == BlockCrubstone.block.getDefaultState().getBlock()
									|| ground2 == BlockCrubstone.block.getDefaultState().getBlock())))
						return false;
					IBlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						world.setBlockState(position.down(), BlockCrubstone.block.getDefaultState(), 2);
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == BlockCrubstone.block.getDefaultState().getBlock()
					|| blockType == BlockCrubstone.block.getDefaultState().getBlock();
		}

		@Override
		protected void setDirtAt(World world, BlockPos pos) {
			if (world.getBlockState(pos).getBlock() != BlockCrubstone.block.getDefaultState().getBlock())
				this.setBlockAndNotifyAdequately(world, pos, BlockCrubstone.block.getDefaultState());
		}
