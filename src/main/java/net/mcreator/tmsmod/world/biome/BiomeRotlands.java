...
import net.mcreator.tmsmod.block.BlockRotleaf;
import net.mcreator.tmsmod.block.BlockRotLog;
import net.mcreator.tmsmod.block.BlockRot;
...
public class BiomeRotlands extends ElementsTmsModMod.ModElement {
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Rotlands").setRainfall(0.5F).setBaseHeight(0.1F).setWaterColor(-13434829).setHeightVariation(0.2F)
					.setTemperature(0.5F));
			setRegistryName("rotlands");
			topBlock = BlockRot.block.getDefaultState();
			fillerBlock = BlockHardRot.block.getDefaultState();
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockRot.block.getDefaultState().getBlock() || ground == BlockHardRot.block.getDefaultState().getBlock())
							&& (ground2 == BlockRot.block.getDefaultState().getBlock()
									|| ground2 == BlockHardRot.block.getDefaultState().getBlock())))
						return false;
					IBlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						world.setBlockState(position.down(), BlockHardRot.block.getDefaultState(), 2);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == BlockRotleaf.block.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, BlockRotleaf.block.getDefaultState());
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.AIR.getDefaultState().getBlock()
									|| state.getBlock() == BlockRotleaf.block.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), BlockRotLog.block.getDefaultState());
								if (genh > 0) {
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockRotLog.block.getDefaultState().getBlock()
					|| blockType == BlockRotleaf.block.getDefaultState().getBlock() || blockType == BlockRot.block.getDefaultState().getBlock()
					|| blockType == BlockHardRot.block.getDefaultState().getBlock();
		}

		@Override
		protected void setDirtAt(World world, BlockPos pos) {
			if (world.getBlockState(pos).getBlock() != BlockHardRot.block.getDefaultState().getBlock())
				this.setBlockAndNotifyAdequately(world, pos, BlockHardRot.block.getDefaultState());
		}
