...
public class BiomeKrinkelWoods extends ElementsTmsModMod.ModElement {
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Krinkel Woodlands").setRainfall(0.8F).setBaseHeight(0.1F).setWaterColor(-16737946)
					.setHeightVariation(0.1F).setTemperature(0.5F));
			setRegistryName("krinkel_woods");
			...
			this.spawnableCreatureList.add(new SpawnListEntry(EntityLlama.class, 10, 5, 15));
	...
	static class CustomTree extends WorldGenAbstractTree {
					...
					if (position.getY() < world.getHeight() - height - 1) {
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.AIR.getDefaultState().getBlock()
									|| state.getBlock() == BlockKrinkelLeaf.block.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), BlockKrinkelLog.block.getDefaultState());
								if (genh > 0) {
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockKrinkelLog.block.getDefaultState().getBlock()
					|| blockType == BlockKrinkelLeaf.block.getDefaultState().getBlock() || blockType == Blocks.GRASS.getDefaultState().getBlock()
