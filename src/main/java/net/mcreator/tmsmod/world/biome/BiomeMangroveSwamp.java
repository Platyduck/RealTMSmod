...
import net.mcreator.tmsmod.block.BlockNatureLeaves;
import net.mcreator.tmsmod.block.BlockMud;
import net.mcreator.tmsmod.block.BlockMangroveLog;
import net.mcreator.tmsmod.ElementsTmsModMod;

import java.util.Random;

@ElementsTmsModMod.ModElement.Tag
public class BiomeMangroveSwamp extends ElementsTmsModMod.ModElement {
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Mangrove Swamp").setRainfall(0.5F).setBaseHeight(0.1F).setWaterColor(-12953290).setHeightVariation(0.2F)
					.setTemperature(0.5F));
			setRegistryName("mangrove_swamp");
			topBlock = BlockMud.block.getDefaultState();
			fillerBlock = BlockMud.block.getDefaultState();
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockMud.block.getDefaultState().getBlock() || ground == BlockMud.block.getDefaultState().getBlock())
							&& (ground2 == BlockMud.block.getDefaultState().getBlock() || ground2 == BlockMud.block.getDefaultState().getBlock())))
						return false;
					IBlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						world.setBlockState(position.down(), BlockMud.block.getDefaultState(), 2);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == BlockNatureLeaves.block.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, BlockNatureLeaves.block.getDefaultState());
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.AIR.getDefaultState().getBlock()
									|| state.getBlock() == BlockNatureLeaves.block.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), BlockMangroveLog.block.getDefaultState());
								if (genh > 0) {
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockMangroveLog.block.getDefaultState().getBlock()
					|| blockType == BlockNatureLeaves.block.getDefaultState().getBlock() || blockType == BlockMud.block.getDefaultState().getBlock()
					|| blockType == BlockMud.block.getDefaultState().getBlock();
		}

		@Override
		protected void setDirtAt(World world, BlockPos pos) {
			if (world.getBlockState(pos).getBlock() != BlockMud.block.getDefaultState().getBlock())
				this.setBlockAndNotifyAdequately(world, pos, BlockMud.block.getDefaultState());
		}
