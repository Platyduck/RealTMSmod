...
import net.mcreator.tmsmod.block.BlockPlasticTaigaGrass;
import net.mcreator.tmsmod.block.BlockPlasticNeedles;
import net.mcreator.tmsmod.block.BlockPlasticLogSpruce;
import net.mcreator.tmsmod.block.BlockPlasticDirt;
import net.mcreator.tmsmod.ElementsTmsModMod;

import java.util.Random;

@ElementsTmsModMod.ModElement.Tag
public class BiomePlasticTaiga extends ElementsTmsModMod.ModElement {
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Artificial Taiga").setRainfall(0.6F).setBaseHeight(0.1F).setWaterColor(-7478785).setHeightVariation(0.2F)
					.setTemperature(0F));
			setRegistryName("plastic_taiga");
			topBlock = BlockPlasticTaigaGrass.block.getDefaultState();
			fillerBlock = BlockPlasticDirt.block.getDefaultState();
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockPlasticTaigaGrass.block.getDefaultState().getBlock()
							|| ground == BlockPlasticDirt.block.getDefaultState().getBlock())
							&& (ground2 == BlockPlasticTaigaGrass.block.getDefaultState().getBlock()
									|| ground2 == BlockPlasticDirt.block.getDefaultState().getBlock())))
						return false;
					IBlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						world.setBlockState(position.down(), BlockPlasticDirt.block.getDefaultState(), 2);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == BlockPlasticNeedles.block.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, BlockPlasticNeedles.block.getDefaultState());
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.AIR.getDefaultState().getBlock()
									|| state.getBlock() == BlockPlasticNeedles.block.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), BlockPlasticLogSpruce.block.getDefaultState());
								if (genh > 0) {
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockPlasticLogSpruce.block.getDefaultState().getBlock()
					|| blockType == BlockPlasticNeedles.block.getDefaultState().getBlock()
					|| blockType == BlockPlasticTaigaGrass.block.getDefaultState().getBlock()
					|| blockType == BlockPlasticDirt.block.getDefaultState().getBlock();
		}

		@Override
		protected void setDirtAt(World world, BlockPos pos) {
			if (world.getBlockState(pos).getBlock() != BlockPlasticDirt.block.getDefaultState().getBlock())
				this.setBlockAndNotifyAdequately(world, pos, BlockPlasticDirt.block.getDefaultState());
		}
