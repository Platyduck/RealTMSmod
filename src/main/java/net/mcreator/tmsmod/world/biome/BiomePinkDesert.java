...
import net.mcreator.tmsmod.block.BlockPinkSandstone;
...
public class BiomePinkDesert extends ElementsTmsModMod.ModElement {
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Pink Desert").setRainfall(0.5F).setBaseHeight(0.1F).setHeightVariation(0F).setTemperature(0.6F));
			setRegistryName("pink_desert");
			topBlock = BlockPinkSand.block.getDefaultState();
			fillerBlock = BlockPinkSandstone.block.getDefaultState();
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockPinkSand.block.getDefaultState().getBlock()
							|| ground == BlockPinkSandstone.block.getDefaultState().getBlock())
							&& (ground2 == BlockPinkSand.block.getDefaultState().getBlock()
									|| ground2 == BlockPinkSandstone.block.getDefaultState().getBlock())))
						return false;
					IBlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						world.setBlockState(position.down(), BlockPinkSandstone.block.getDefaultState(), 2);
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == BlockPinkSand.block.getDefaultState().getBlock()
					|| blockType == BlockPinkSandstone.block.getDefaultState().getBlock();
		}

		@Override
		protected void setDirtAt(World world, BlockPos pos) {
			if (world.getBlockState(pos).getBlock() != BlockPinkSandstone.block.getDefaultState().getBlock())
				this.setBlockAndNotifyAdequately(world, pos, BlockPinkSandstone.block.getDefaultState());
		}
