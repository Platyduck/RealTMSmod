...
import net.mcreator.tmsmod.entity.EntityHowler;
import net.mcreator.tmsmod.block.BlockWitherRose;
import net.mcreator.tmsmod.block.BlockTilename;
...
public class BiomeMomillaVoid extends ElementsTmsModMod.ModElement {
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Momilla Void").setRainfall(0F).setBaseHeight(0.1F).setWaterColor(-16777216).setHeightVariation(0F)
					.setTemperature(-1F));
			setRegistryName("momilla_void");
			topBlock = BlockTilename.block.getDefaultState();
			fillerBlock = BlockGlitchLeaf.block.getDefaultState();
			...
			this.spawnableCreatureList.add(new SpawnListEntry(EntityHowler.EntityCustom.class, 5, 1, 1));
		}
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockTilename.block.getDefaultState().getBlock() || ground == BlockGlitchLeaf.block.getDefaultState().getBlock())
							&& (ground2 == BlockTilename.block.getDefaultState().getBlock()
									|| ground2 == BlockGlitchLeaf.block.getDefaultState().getBlock())))
						return false;
					IBlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						world.setBlockState(position.down(), BlockGlitchLeaf.block.getDefaultState(), 2);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == BlockGlitchLeaf.block.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, BlockGlitchLeaf.block.getDefaultState());
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.AIR.getDefaultState().getBlock()
									|| state.getBlock() == BlockGlitchLeaf.block.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), BlockMomillaLog.block.getDefaultState());
								if (genh > 0) {
								...
								for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										this.setBlockAndNotifyAdequately(world,
												...
												BlockWitherRose.block.getDefaultState());
									}
								}
							}
						}
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockMomillaLog.block.getDefaultState().getBlock()
					|| blockType == BlockGlitchLeaf.block.getDefaultState().getBlock()
					|| blockType == BlockTilename.block.getDefaultState().getBlock()
					|| blockType == BlockGlitchLeaf.block.getDefaultState().getBlock();
		}

		@Override
		protected void setDirtAt(World world, BlockPos pos) {
			if (world.getBlockState(pos).getBlock() != BlockGlitchLeaf.block.getDefaultState().getBlock())
				this.setBlockAndNotifyAdequately(world, pos, BlockGlitchLeaf.block.getDefaultState());
		}
