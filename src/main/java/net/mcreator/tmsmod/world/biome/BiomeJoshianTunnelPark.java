...
import net.mcreator.tmsmod.block.BlockPlasticLog;
import net.mcreator.tmsmod.block.BlockPlasticLeaves;
...
public class BiomeJoshianTunnelPark extends ElementsTmsModMod.ModElement {
	...
	static class BiomeGenCustom extends Biome {
		public BiomeGenCustom() {
			super(new Biome.BiomeProperties("Artificial Park").setRainfall(0.5F).setBaseHeight(0.1F).setWaterColor(-14810881).setHeightVariation(0.2F)
					.setTemperature(0.5F));
			setRegistryName("joshian_tunnel_park");
			topBlock = BlockPlasticGrass.block.getDefaultState();
	...
	static class CustomTree extends WorldGenAbstractTree {
		...
		public boolean generate(World world, Random rand, BlockPos position) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BlockPlasticGrass.block.getDefaultState().getBlock()
							|| ground == BlockPlasticDirt.block.getDefaultState().getBlock())
							&& (ground2 == BlockPlasticGrass.block.getDefaultState().getBlock()
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getBlock().isLeaves(state, world, blockpos)
												...
												|| state.getBlock() == BlockPlasticLeaves.block.getDefaultState().getBlock()) {
											this.setBlockAndNotifyAdequately(world, blockpos, BlockPlasticLeaves.block.getDefaultState());
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							if (state.getBlock().isAir(state, world, genhPos) || state.getBlock() == Blocks.AIR.getDefaultState().getBlock()
									|| state.getBlock() == BlockPlasticLeaves.block.getDefaultState().getBlock()) {
								this.setBlockAndNotifyAdequately(world, position.up(genh), BlockPlasticLog.block.getDefaultState());
								if (genh > 0) {
		...
		protected boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == BlockPlasticLog.block.getDefaultState().getBlock()
					|| blockType == BlockPlasticLeaves.block.getDefaultState().getBlock()
					|| blockType == BlockPlasticGrass.block.getDefaultState().getBlock()
