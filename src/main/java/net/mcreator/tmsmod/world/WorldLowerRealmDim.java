...
import net.mcreator.tmsmod.item.ItemLowerRealmDim;
import net.mcreator.tmsmod.block.BlockLowerStone;
import net.mcreator.tmsmod.block.BlockHubWall;
...
public class WorldLowerRealmDim extends ElementsTmsModMod.ModElement {
	public static int DIMID = 19;
	public static final boolean NETHER_TYPE = false;
	@GameRegistry.ObjectHolder("tms_mod:lower_realm_dim_portal")
	...
	public void initElements() {
		...
		elements.items.add(() -> new ItemLowerRealmDim().setUnlocalizedName("lower_realm_dim").setRegistryName("lower_realm_dim"));
	}
	...
	public void preInit(FMLPreInitializationEvent event) {
		if (DimensionManager.isDimensionRegistered(DIMID)) {
			...
			System.err.println("Dimension ID for dimension lower_realm_dim is already registered. Falling back to ID: " + DIMID);
		}
		dtype = DimensionType.register("lower_realm_dim", "_lower_realm_dim", DIMID, WorldProviderMod.class, true);
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(ItemLowerRealmDim.block, 0, new ModelResourceLocation("tms_mod:lower_realm_dim", "inventory"));
	}
					...
					for (int l7 = 1; l7 < 3; ++l7) {
						for (int k8 = -1; k8 < 3; ++k8) {
							...
							this.world.setBlockState(new BlockPos(k9, k10, k11),
									flag ? BlockHubWall.block.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
				...
				for (int l8 = 0; l8 < 4; ++l8) {
					for (int l9 = -1; l9 < 4; ++l9) {
						...
						this.world.setBlockState(new BlockPos(l10, l11, k12),
								flag1 ? BlockHubWall.block.getDefaultState().getBlock().getDefaultState() : iblockstate, 2);
					}
				}
	...
	public static class BlockCustomPortal extends BlockPortal {
		public BlockCustomPortal() {
			setHardness(-1.0F);
			setUnlocalizedName("lower_realm_dim_portal");
			setRegistryName("lower_realm_dim_portal");
			setLightLevel(0.2F);
		}
		...
		public static class Size {
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, EnumFacing p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock())
							|| this.world.getBlockState(blockpos.down()).getBlock() != BlockHubWall.block.getDefaultState().getBlock()) {
						break;
					}
				}
				Block block = this.world.getBlockState(p_180120_1_.offset(p_180120_2_, i)).getBlock();
				return block == BlockHubWall.block.getDefaultState().getBlock() ? i : 0;
			}
			...
			protected int calculatePortalHeight() {
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (block != BlockHubWall.block.getDefaultState().getBlock()) {
								break label56;
							}
				...
				for (int j = 0; j < this.width; ++j) {
					if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height)).getBlock() != BlockHubWall.block
	...
	public static class ChunkProviderModded implements IChunkGenerator {
		private static final IBlockState STONE = BlockLowerStone.block.getDefaultState();
		private static final IBlockState STONE2 = BlockLowerStone.block.getDefaultState();
	...
	public static class GenLayerBiomesCustom extends GenLayer {
		private Biome[] allowedBiomes = {Biome.REGISTRY.getObject(new ResourceLocation("tms_mod:lower_world")),};
